...
export default function E_Waste_Submission_Admin() {
    ...
    const loadTable = async (page = 1, limit = 5, searchTerm = "") => {
        try {
            const res = await E_Waste_Submission_API.fetchAll(page, limit, searchTerm);
    ...
    const deleteUsers = async (id) => {
        const confirmDelete = window.confirm(
            "Are you sure you want to delete this E-Waste Submission ?"
        );

        if (!confirmDelete) return;

        try {
            await E_Waste_Submission_API.Delete(id);   // ✅ API call
            ...
            setSuccessMsg("E-Waste Submission deleted successfully.");
            ...
            <>
                ...
                <div className="table-responsive">
                    <form id="load-data" onSubmit={(e) => e.preventDefault()}>
                        ...
                        <Admin_Table
                            columns={["ID", "User", "Category", "Brand", "Model", "User Cond.", "Final Cond.", "Type", "Facility", "Address", "Date/Time", "Phone", "Notes", "Status", "Created At", "Updated At", "Action"]}
                        >
                            {Fatch_E_Waste_Submission_Table(rows, edit_Users, deleteUsers, deletingId)}
                        </Admin_Table>
