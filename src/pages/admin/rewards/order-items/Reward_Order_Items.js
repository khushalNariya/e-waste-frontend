...
import { Reward_Order_Items_API } from "./api";
import { Fetch_Reward_Order_Items_Table } from "./Table";
...
export default function Reward_Order_Items_admin() {
    ...
    const loadTable = async (page = 1, limit = 5, searchTerm = "") => {
        try {
            const res = await Reward_Order_Items_API.fetchAll(page, limit, searchTerm);
    ...
    const deleteItem = async (id) => {
        ...
        try {
            await Reward_Order_Items_API.Delete(id);
            setDeletingId(id);
            ...
            setSuccessMsg("Order Item deleted successfully.");
        } catch (error) {
        ...
        <div className="admin-content p-3">
            ...
            <div className="table-responsive">
                <form id="load-data" onSubmit={(e) => e.preventDefault()}>
                    <Admin_Table
                        columns={["ID", "Order ID", "Product", "Quantity", "Points", "Subtotal", "Created At", "Action"]}
                    >
                        {Fetch_Reward_Order_Items_Table(rows, null, deleteItem, deletingId)}
                    </Admin_Table>
