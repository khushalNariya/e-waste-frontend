...
import { Reward_Order_Address_API } from "./api";
import { Fetch_Reward_Order_Address_Table } from "./Table";
...
export default function Reward_Order_Address_admin() {
    ...
    const loadTable = async (page = 1, limit = 5, searchTerm = "") => {
        try {
            const res = await Reward_Order_Address_API.fetchAll(page, limit, searchTerm);
    ...
    const deleteAddress = async (id) => {
        ...
        try {
            await Reward_Order_Address_API.Delete(id);
            setDeletingId(id);
    ...
    return (
        <div className="admin-content p-3">
            <Admin_List_Header
                title="Reward Order Address List"
            ...
            <div className="table-responsive">
                <form id="load-data" onSubmit={(e) => e.preventDefault()}>
                    <Admin_Table
                        columns={["ID", "Order ID", "Name", "Address", "City", "State", "Pincode", "Action"]}
                    >
                        {Fetch_Reward_Order_Address_Table(rows, null, deleteAddress, deletingId)}
                    </Admin_Table>
