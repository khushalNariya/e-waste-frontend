...
export function Fetch_Reward_Replace_Request_Table(rows, deleteRequest, deletingId, handleStatusUpdate, viewDetails) {
    if (!rows || rows.length === 0) {
        return (
            <tr>
                <td colSpan="8" className="text-center text-muted">
                    No Replace Requests Found
                </td>
            </tr>
        );
    }
        ...
        <tr key={u.id} className={deletingId === u.id ? "fade-row" : ""}>
            ...
            <td>
                <span className="badge bg-secondary text-white" style={{ fontSize: "11px" }}>
                    {u.replace_type?.replace("_", " ").toUpperCase() || "N/A"}
                </span>
            </td>
            <td>
                <select
                    className={`badge ${statusMap[u.replace_status]?.bg || "bg-secondary"}`}
                    value={u.replace_status}
                ...
                >
                    {Object.entries(statusMap).map(([key, value]) => (
                        <option
                            key={key}
                            value={key}
                            style={{ background: "white", color: "black", padding: "10px", fontSize: "13px" }}
                        >
                            {value.text}
                        </option>
                    ))}
