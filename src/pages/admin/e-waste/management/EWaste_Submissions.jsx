...
import { EWaste_Submission_API } from "./api";
import { Fetch_EWaste_Submission_Table } from "./Table";
import { calculatePagination, generatePaginationButtons } from "../../shared/Pagination/Pagination";
import { useNavigate } from "react-router-dom";

export default function EWaste_Submissions() {
    ...
    const loadTable = async (page = 1, limit = 5, searchTerm = "") => {
        try {
            const res = await EWaste_Submission_API.fetchAll(page, limit, searchTerm);
    ...
    const handleEdit = (id) => {
        sessionStorage.setItem("edit_submission_id", id);
        // navigate("/admin/Edit-EWaste-Submission");
    };

    const handleDelete = async (id) => {
        if (!window.confirm("Are you sure you want to delete this submission?")) return;
        try {
            await EWaste_Submission_API.Delete(id);
            setDeletingId(id);
            setTimeout(() => {
                setRows(prev => prev.filter(row => row.submission_id !== id));
                setDeletingId(null);
    ...
    return (
        <div className="admin-content p-3">
            <div className="d-flex justify-content-between align-items-center mb-3">
                <h4 className="fw-bold">E-Waste Submissions</h4>
            </div>
            ...
            <div className="table-responsive">
                <table className="table table-bordered table-hover">
                    ...
                    <tbody>
                        {Fetch_EWaste_Submission_Table(rows, handleEdit, handleDelete, deletingId)}
                    </tbody>
                </table>
