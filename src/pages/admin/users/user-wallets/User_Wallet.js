...
import { User_Wallet_API } from "./api";
import { Fatch_User_Wallet_Table } from "./Table";
...
export default function User_Wallet_Admin() {
    ...
    const loadTable = async (page = 1, limit = 10, searchTerm = "") => {
        try {
            const res = await User_Wallet_API.fetchAll(page, limit, searchTerm);
            ...
            <>
                ...
                <div className="table-responsive">
                    <form id="load-data" onSubmit={(e) => e.preventDefault()}>
                        <Admin_Table
                            columns={["ID", "User ID", "User Name", "Total Points", "Created At", "Updated At"]}
                        >
                            {Fatch_User_Wallet_Table(rows, null, null)}
                        </Admin_Table>
