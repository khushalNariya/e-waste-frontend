...
import { Reward_Return_Pickup_API } from "./api";
import { Fetch_Reward_Return_Pickup_Table } from "./Table";
...
export default function Reward_Return_Pickups_Admin() {
    ...
    const loadTable = async (page = 1, limit = 5, searchTerm = "", filters = {}) => {
        try {
            const res = await Reward_Return_Pickup_API.fetchAll(page, limit, searchTerm, filters);
    ...
    return (
        <div className="admin-content p-3">
            <Admin_List_Header
                title="Return Pickups Courier Schedule"
            ...
            <div className="table-responsive">
                <form id="load-data" onSubmit={(e) => e.preventDefault()}>
                    <Admin_Table
                        columns={["ID", "Return Number", "Order Number", "User Name", "Address Details", "Courier Status"]}
                    >
                        {Fetch_Reward_Return_Pickup_Table(rows)}
                    </Admin_Table>
