...
import { Reward_Replace_Item_API } from "./api";
import { Fetch_Reward_Replace_Item_Table } from "./Table";
...
export default function Reward_Replace_Items_Admin() {
    ...
    const loadTable = async (page = 1, limit = 5, searchTerm = "") => {
        try {
            const res = await Reward_Replace_Item_API.fetchAll(page, limit, searchTerm);
        ...
        <div className="admin-content p-3">
            ...
            <div className="table-responsive">
                <form id="load-data" onSubmit={(e) => e.preventDefault()}>
                    <Admin_Table
                        columns={["ID", "Replace Number", "Order Number", "Product Details", "Quantity", "Points/Item", "Total Points", "Item Condition"]}
                    >
                        {Fetch_Reward_Replace_Item_Table(rows)}
                    </Admin_Table>
